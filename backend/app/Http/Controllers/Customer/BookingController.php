...
class BookingController extends Controller
{
    public function store(Request $request)
    {
        ...
        // UNLESS service_type is 'barang' (barang bookings don't need seats, only bagasi)
        if ($isCar) {
            $serviceType = $ride->service_type ?? null;
            $isBarangOnly = ($serviceType === 'barang');
            
            // Only check seats if not barang-only service
            if (!$isBarangOnly) {
        ...
        if ($isCar) {
            return app()->call(\App\Http\Controllers\Customer\BookingMobilController::class . '@store', ['request' => $request, 'ride' => $ride]);
        }

        if ($isTitipBarang) {
            return app()->call(\App\Http\Controllers\Customer\BookingTitipBarangController::class . '@store', ['request' => $request, 'ride' => $ride]);
        }

        if ($isBarang) {
            return app()->call(\App\Http\Controllers\Customer\BookingBarangController::class . '@store', ['request' => $request, 'ride' => $ride]);
        }

        // default -> motor
        return app()->call(\App\Http\Controllers\Customer\BookingMotorController::class . '@store', ['request' => $request, 'ride' => $ride]);
    }
