...
class RefundController extends Controller
{
    ...
    public function checkEligibility(Request $request, $bookingId)
    {
        ...
        // Check if booking was cancelled by mitra
        $isMitraCancellation = false;
        if ($booking->status === 'cancelled' && 
            isset($booking->cancellation_reason) && 
            str_contains($booking->cancellation_reason, 'mitra')) {
            $isMitraCancellation = true;
        }
        
        // No admin fee if cancelled by mitra, otherwise 20% admin fee
        $adminFee = $isMitraCancellation ? 0 : ($totalAmount * 0.20);
