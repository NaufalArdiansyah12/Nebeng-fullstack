...
import '../widgets/barang_location_section.dart';
import '../widgets/barang_selection_card.dart';
import '../widgets/barang_vehicle_widgets.dart';
import '../widgets/barang_transportation_dialog.dart';
import '../utils/barang_helpers.dart';

class CreateBarangRidePage extends StatefulWidget {
  const CreateBarangRidePage({Key? key}) : super(key: key);

  @override
  State<CreateBarangRidePage> createState() => _CreateBarangRidePageState();
}

class _CreateBarangRidePageState extends State<CreateBarangRidePage> {
  ...
  Future<void> _showVehicleDetailsDialog() async {
    ...
    try {
      ...
      // Show all vehicles for barang (no filter)
    } catch (_) {
      vehicles = [];
    }
    ...
    if (selected != null) {
      ...
      _seatsController.text = (selected['seats'] ?? 0).toString();
  ...
  Future<void> _goToDetailPage() async {
    ...
    if (_selectedBagasiCapacity == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Mohon pilih kapasitas bagasi')),
      );
      return;
    }
          ...
          children: [
            ...
            BarangSelectionCard(
              ...
              subtitle: _selectedDate != null
                  ? BarangHelpers.formatDate(_selectedDate!)
                  : null,
              onTap: _selectDate,
            ),
            const SizedBox(height: 12),
            BarangSelectionCard(
              ...
              subtitle: _selectedTime != null
                  ? BarangHelpers.formatTime(_selectedTime!)
                  : null,
              onTap: _selectTime,
            ),
            const SizedBox(height: 12),
            BarangSelectionCard(
              ...
              subtitle: BarangHelpers.getBagasiLabel(_selectedBagasiCapacity),
