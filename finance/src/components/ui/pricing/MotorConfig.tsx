...
import { Save, Bike, Users, Package, LayoutGrid } from "lucide-react";
...
const MotorConfig = () => {
  ...
  const [config, setConfig] = useState<MotorConfigData>({
  ...
  const fetchConfig = async () => {
    try {
      setLoading(true);
      const res = await api.get("/finance/pricing-config/motor");
      const data = res.data.data;
      const newConfig: MotorConfigData = {
        hanya_tebengan: { base_price: 0, price_per_km: 0, min_price: 0 },
        hanya_barang: {
          weight_rules: [
            { category: "kecil", price_per_kg: 0 },
      ...
      console.error(err);
      toast.error("Gagal memuat konfigurasi motor");
    } finally {
      setLoading(false);
    }
  };
        ...
        configs: [
          {
            ...
            base_price: roundNearest(config.hanya_tebengan.base_price),
            price_per_km: roundNearest(config.hanya_tebengan.price_per_km),
            min_price: roundNearest(config.hanya_tebengan.min_price),
          },
          {
            service_type: "hanya_barang",
            base_price: 0,
            price_per_km: 0,
            weight_rules: config.hanya_barang.weight_rules.map(w => ({ category: w.category, price_per_kg: roundNearest(w.price_per_kg) })),
          },
          {
            service_type: "tebengan_dan_barang",
            base_price: roundNearest(config.tebengan_dan_barang.base_price),
            price_per_km: roundNearest(config.tebengan_dan_barang.price_per_km),
            weight_rules: config.tebengan_dan_barang.weight_rules.map(w => ({ category: w.category, price_per_kg: roundNearest(w.price_per_kg) })),
          },
        ],
      };
      await api.put("/finance/pricing-config/motor", payload);
      toast.success("Konfigurasi motor berhasil disimpan");
      fetchConfig();
    } catch (err: any) {
      console.error(err);
