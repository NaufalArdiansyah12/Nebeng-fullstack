...
interface BankData {
  ...
  bank_account_photo: string | null;
  status: 'pending' | 'approved' | 'rejected' | null;
  rejection_reason: string | null;
  verified_at: string | null;
  created_at: string;
...
export const MitraProvider = ({ children }: { children: ReactNode }) => {
  ...
  const getMitraDetail = async (id: string): Promise<MitraDetailData | null> => {
    try {
      ...
      console.log('🖼️ Photo data from API:', {
        profile_photo: detail.profile_photo,
        ktp_photo: detail.ktp_data?.photo_ktp,
        sim_photo: detail.sim_data?.sim_photo,
        skck_photo: detail.skck_data?.skck_photo,
        bank_photo: detail.bank_data?.bank_account_photo
      });
      ...
      const transformedDetail: MitraDetailData = {
        ...
        bank_data: detail.bank_data || null,
