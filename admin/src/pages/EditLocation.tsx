...
const EditLocation = () => {
  ...
  const loadLocationData = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || err?.message || "Gagal memuat data lokasi";
      alert(msg);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Terminal berhasil diupdate");
      navigate("/dashboard/pos-mitra-by-location");
    } catch (err: any) {
      console.error("Failed to update location:", err);
      const msg = err?.response?.data?.message || err?.message || "Gagal mengupdate lokasi";
      alert(msg);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Memuat data terminal...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold">Edit Terminal</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Perbarui informasi terminal pos mitra
            </p>
          </div>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex gap-3 pt-4 border-t">
                <Button type="submit" disabled={loading} className="flex-1 h-11">
                  {loading ? 'Menyimpan...' : 'Update Terminal'}
                </Button>
                <Button 
                  type="button" 
                  variant="outline" 
