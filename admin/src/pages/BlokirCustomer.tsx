...
const BlokirCustomer = () => {
  ...
  const filteredData = useMemo(() => {
    return blockedCustomers.filter((customer) => {
  ...
  }, [blockedCustomers, searchQuery, selectedDate]);
  ...
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Blokir Customer");

    XLSX.writeFile(workbook, `blokir-customer-${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };
    ...
    <div className="space-y-6">
      <Card className="shadow-sm">
        <CardHeader className="pb-4">
          <CardTitle className="text-xl font-semibold">Daftar Blockir Data Customer</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Filters */}
                ...
                {paginatedData.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={6} className="py-8 text-center text-muted-foreground">
                      Tidak ada customer yang diblokir
                    </td>
                  </tr>
                )}
              </tbody>
