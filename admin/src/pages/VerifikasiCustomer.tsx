...
const VerifikasiCustomer = () => {
  ...
  const { verifikasiCustomerList, loading, error, updateVerifikasiCustomerStatus } = useVerifikasi();
  ...
  }, [verifikasiCustomerList, searchQuery, selectedDate, statusFilter]);
  ...
  const handleDownload = () => {
    ...
    const excelData = dataToExport.map(customer => ({
      ...
      "NIK": customer.nikCustomer,
      "TANGGAL LAHIR": format(customer.tanggalLahir, "dd-MM-yyyy"),
      "STATUS": customer.status.toUpperCase() === "APPROVED" ? "TERVERIFIKASI" : 
                customer.status.toUpperCase() === "REJECTED" ? "DITOLAK" :
                customer.status.toUpperCase() === "PENDING" ? "PENGAJUAN" : customer.status,
      "TANGGAL PENGAJUAN": format(customer.tanggalPengajuan, "dd-MM-yyyy")
    }));
    ...
    XLSX.writeFile(workbook, `data-verifikasi-customer-${format(new Date(), "yyyy-MM-dd")}.xlsx`);
  };
  ...
  const handleConfirmBlock = () => {
    if (selectedCustomerId && blockCustomer) {
  ...
  const handleConfirmUnblock = () => {
    if (selectedCustomerId && unblockCustomer) {
    ...
    <div className="space-y-6">
      <Card className="shadow-sm">
        <CardHeader className="pb-4">
          <CardTitle className="text-xl font-semibold">Data Customer</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Filters */}
                ...
                {loading ? (
                  ...
                  paginatedData.map((customer, index) => (
                    <tr key={index} className="border-b border-border/50 hover:bg-muted/30">
                      ...
                      <td className="py-4 px-4">{customer.namaLengkap}</td>
                      <td className="py-4 px-4 text-primary">{customer.nikCustomer}</td>
                      <td className="py-4 px-4">{format(customer.tanggalLahir, 'dd MMM yyyy', { locale: localeId })}</td>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center justify-center gap-2">
                          <Button 
                            ...
                            onClick={() => navigate(`/dashboard/verifikasi-costumer/${customer.id}`)}
                          >
...
export default VerifikasiCustomer;